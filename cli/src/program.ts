...
import { registerApply, registerDiff } from './commands/reconcile.js';
 ...
 * text, but each action is a no-op stub (see `lib/stub.ts`). This is the
 * integration starting point: implement a command by replacing its `stub(...)`
 * call with the real behavior — no wiring changes required.
 */
export function buildProgram(): Command {
  ...
  program
    .name('mdk')
    .description(
      'MDK command-line tool (scaffold). The command surface is complete; commands are stubs.',
    )
